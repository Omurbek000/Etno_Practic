...
import type { Cartoon, Genre } from '../types';
...
export default function Cartoons() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get('/cartoon/', { params: p }).then(r => { setItems(r.data.results || r.data); setTotal(r.data.count || 0); }).finally(() => setLoading(false));
  ...
  return (<div>
    ...
    <div className="grid">
      {loading ? Array.from({ length: 10 }).map((_, i) => <div key={i} className="skeleton" />) : items.map(c => <MovieCard key={c.id} id={c.id} title={c.title} posterUrl={c.cartoon_image || ''} accessType={c.access_type} type="cartoon" />)}
    </div>
